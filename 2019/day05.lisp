...
(defun op-add (meta)
  (lambda (memory ip)
    (let* ((left (get-value memory (aref memory (+ ip 1)) (immediatep meta 1)))
           (right (get-value memory (aref memory (+ ip 2)) (immediatep meta 2)))
           (value (+ left right)) 
           (pos (aref memory (+ ip 3))))
...
(defun op-in ()
  (lambda (memory ip)
    (let ((value (pop *input*))
          (pos (aref memory (+ ip 1))))
      (write-value memory pos value)
      (+ ip 2))))

(defun op-out (meta)
  (lambda (memory ip)
    (let* ((left (aref memory (+ ip 1)))
           (value (get-value memory left (immediatep meta 1))))
      (push value *output*)
      (+ ip 2))))
...
(defun parse-op (memory ip &aux (value (aref memory ip)))
