...
(defun frequencies (str)
  (labels ((frequencies-recursive (lst acc)
             (if (not lst)
               acc
               (let ((next (car lst)))
                 (if (not (gethash next acc))
                   (setf (gethash next acc) 0))
                 (setf (gethash next acc)
                       (+ 1 (gethash next acc)))
                 (frequencies-recursive (cdr lst) acc)))))
    (frequencies-recursive (coerce str 'list) (make-hash-table)))) ; XXX can we avoid to `coerce` this?


(defun hash-contains (value hash)
  (loop for key being the hash-keys of hash
        when (eql value (gethash key hash))
        collect it))


(defun solve-day2-1 (ids)
  (let ((twos 0)
        (threes 0))
    (loop for id in ids
          do (let ((freq (frequencies id)))
                (progn
                  (if (hash-contains 2 freq) (setf twos (+ 1 twos)))
                  (if (hash-contains 3 freq) (setf threes (+ 1 threes))))))
    (* twos threes)))


(defun day2-1 ()
  (let* ((in (open "./day2.input"))
         (lst (read-by-line in))
         (result (solve-day2-1 lst)))
    (close in)
    result))


(defun off-by-onep (str1 str2)
  (labels ((differences (lst1 lst2 acc)
             (if (not lst1)
               acc
               (if (eql (car lst1) (car lst2))
                 (differences (cdr lst1) (cdr lst2) acc)
                 (differences (cdr lst1) (cdr lst2) (+ acc 1))))))
    (eql 1 (differences (coerce str1 'list)
                        (coerce str2 'list)
                        0))))


(defun string-intersection (str1 str2)
  (labels ((recurse (lst1 lst2)
             (if lst1
               (if (eql (car lst1) (car lst2))
                 (cons (car lst1)
                       (recurse (cdr lst1) (cdr lst2)))
                 (recurse (cdr lst1) (cdr lst2))))))
    (concatenate 'string (recurse (coerce str1 'list)
                                  (coerce str2 'list)))))


(defun solve-day2-2 (ids)
  (let ((valid nil))
    (loop for id1 in ids
          do (loop for id2 in ids
                   when (off-by-onep id1 id2)
                   do (push (cons id1 id2) valid)))
    (string-intersection (caar valid) (cdar valid))))


(defun day2-2 ()
  (let* ((in (open "./day2.input"))
         (lst (read-by-line in))
         (result (solve-day2-2 lst)))
    (close in)
    result))
